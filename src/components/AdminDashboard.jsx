  ...
  return (
    <div className="flex flex-col md:flex-row gap-0 h-[calc(100vh-72px)] bg-slate-50">
      ...
      <div className="w-full md:w-64 shrink-0 bg-white border-b md:border-b-0 md:border-r border-slate-200 p-4 overflow-y-auto">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-slate-600 bg-brand-100 px-2.5 py-0.5 rounded-full">{activeCity === 'wuhan' ? '武汉' : '长沙'}</span>
            <button onClick={onBack} className="text-xs text-slate-600 hover:text-slate-800 font-medium">返回前台 →</button>
          </div>
        </div>
        ...
        <button
          ...
          className={`w-full mt-4 px-3 py-2 rounded-2xl text-sm font-medium transition-colors flex items-center justify-center gap-1.5 ${
            showSettings ? 'bg-brand-50 text-slate-700 ring-1 ring-brand-200' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
          }`}
        >
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto">
        ...
        {showSettings && (
          <div>
            <h3 className="text-lg font-bold text-slate-800 mb-4">⚙ 页面设置</h3>
        ...
        {!showSettings && activeTabId && currentTab ? (
          <>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-800">{currentTab.label} <span className="text-sm font-normal text-slate-400">({currentItems.length} 条内容)</span></h3>
              ...
              {currentItems.map((item, idx) => (
                <div key={item.id} className="bg-white rounded-2xl border border-slate-100 px-4 py-3 flex items-start justify-between gap-3 hover:border-brand-200 hover:-translate-y-0.5 transition-all duration-200">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => openEditItem(item)} className="px-3 py-1.5 text-xs font-medium text-slate-600 bg-brand-50 hover:bg-brand-100 rounded-3xl transition-colors">编辑</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="relative w-full max-w-lg mx-4 bg-white rounded-2xl shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
              <h2 className="text-lg font-bold text-slate-800">{editingItem ? '编辑' : '新增'}卡片</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={addLink} className="text-xs font-medium text-slate-600 hover:text-slate-700 flex items-center gap-1">
